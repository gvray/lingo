...
const componentsItems: { mode: AppMode; label: string }[] = [
  { mode: "agents", label: "Agents 代理" },
  { mode: "models", label: "Models 模型" },
  { mode: "messages", label: "Messages 消息" },
  { mode: "tools", label: "Tools 工具" },
  { mode: "memory", label: "短期记忆" },
  { mode: "streaming", label: "流式输出" },
  { mode: "structured", label: "结构化输出" },
];
    ...
    <div className="w-64 h-full bg-zinc-900 flex flex-col border-r border-zinc-800">
      ...
      <div className="px-2 py-2 space-y-1">
        ...
        >
          ...
          <svg
            className={`w-4 h-4 transition-transform ${appsExpanded ? "rotate-180" : ""}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        {/* Components Section - dev only */}
        {process.env.NODE_ENV === "development" && (
          <>
            <div className="mx-3 my-2 border-t border-zinc-800" />
            <button
              onClick={() => setComponentsExpanded(!componentsExpanded)}
              className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg transition-colors text-zinc-300 hover:bg-zinc-800 hover:text-white"
            >
              <div className="flex items-center gap-3">
                <svg
                  className="w-5 h-5"
                ...
                <span className="text-sm">组件案例</span>
              </div>
              <svg
                className={`w-4 h-4 transition-transform ${componentsExpanded ? "rotate-180" : ""}`}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
